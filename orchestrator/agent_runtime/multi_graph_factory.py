...
def create_multi_agent_graph():
    ...
    graph.add(
        ...
        depends_on=["implementation_backend", "implementation_frontend", "qa_plan"],
    )

    graph.add("validation", registry.get("validation").factory(), depends_on=["implementation"])
    graph.add("acceptance", registry.get("acceptance").factory(), depends_on=["validation"])
    graph.add("review", registry.get("review").factory(), depends_on=["acceptance"])
    graph.add("release", registry.get("release").factory(), depends_on=["review"])
