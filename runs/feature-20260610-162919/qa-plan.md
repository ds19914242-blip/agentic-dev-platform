...
- **Component-level English leakage**: if `ConfirmModal` has hardcoded English confirm/cancel buttons, they'd remain English despite the page being translated — flagged for inspection; fixing it there could affect other pages, so it should be confirmed before any change and likely left out of this scoped task.
