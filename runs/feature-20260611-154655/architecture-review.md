...
- **Interpolation breakage**: strings mix variables (`${available.length}`, `pendingDelete?.name`, date formatting). Edits must preserve template expressions and JSX braces.
- **Encoding/quotes**: Russian guillemets `«»` are already used in the codebase (line 204); keep UTF-8 and avoid smart-quote mismatches.
- **Consistency of terminology**: ensure a single chosen translation for recurring terms (Collection, Source, Analyze) matches terminology used on already-translated pages to avoid inconsistent UI vocabulary.
