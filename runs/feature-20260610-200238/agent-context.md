...
2. Immediately after the closing `</h1>` (line 29), add a helper paragraph using the established style class:
   ...
   ```
   - Use `text-sm text-slate-500` to match the reports-page primary helper line, or `text-xs text-slate-400` for a smaller subdued line — pick one for consistency with the desir
