...
- Grep the affected files for residual ASCII-letter strings in `error:`/`note:` fields to co
...
- `npx tsc --noEmit` (or the project's typecheck) — pure string edits should not affect types; confirms nothing broke.
- Grep the affected files for residual ASCII-letter strings in `error:`/`note:` fields to confirm no use
